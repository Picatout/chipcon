	...
	public class dasm
	{
		...
		static internal string deassemble(ushort pc, ushort code)
		{
			...
			case 1: // 1NNN saut vers l'adresse NNN
				s=string.Format("{0:S}JP {1:3,X} ",s,((b1&0xf)<<8)|b2);
				break;
			case 2: // 2NNN  appelle la sous-routine à l'adresse NNN
				s=string.Format("{0:S}CALL {1:3,X} ",s,((b1&0xf)<<8)|b2);
				break;
			case 3: // 3XKK     saute l'instruction suivante si VX == KK 
				s=string.Format("{0:S}SE V{1:X}, {2:D} ",s,(b1&0xf),b2);
				break;
			case 4: // 4XKK     Saute l'instruction suivante si VX <> KK 
				s=string.Format("{0:S}SNE V{1:X}, {2:D} ",s,(b1&0xf),b2);
				break;
			case 5: // 5XY0     Saute l'instruction suivante si VX == VY
				s=string.Format("{0:S}SE V{1:X}, V{2:X} ",s,(b1&0xf),(b2&0xf0)>>4);
				break;
			case 6: // 6XKK     VX := KK 
				s=string.Format("{0:S}LD V{1:X}, {2:D} ",s,(b1&0xf),b2);
				break;
			case 7: // 7XKK     VX := VX + KK
				s=string.Format("{0:S}ADD V{1:X}, {2:D} ",s,(b1&0xf),b2);
				break;
			case 8: 
				switch(b2&0xf){
				case 0: // 8XY0     VX := VY
					s=string.Format("{0:S}LD V{1:X}, V{2:D} ",s,(b1&0xf),(b2&0xf0)>>4);
					break;
				case 1: // 8XY1     VX := VX or VY
					s=string.Format("{0:S}OR V{1:X}, V{2:D} ",s,(b1&0xf),(b2&0xf0)>>4);
					break;
				case 2: // 8XY2     VX := VX and VY
					s=string.Format("{0:S}AND V{1:X}, V{2:D} ",s,(b1&0xf),(b2&0xf0)>>4);
					break;
				case 3: // 8XY3     VX := VX xor VY
					s=string.Format("{0:S}XOR V{1:X}, V{2:D} ",s,(b1&0xf),(b2&0xf0)>>4);
					break;
				case 4: // 8XY4     VX := VX + VY, VF := carry 
					s=string.Format("{0:S}ADD V{1:X}, V{2:D} ",s,(b1&0xf),(b2&0xf0)>>4);
					break;
				case 5: // 8XY5     VX := VX - VY, VF := not borrow 
					s=string.Format("{0:S}SUB V{1:X}, V{2:D} ",s,(b1&0xf),(b2&0xf0)>>4);
					break;
				case 6: // 8XY6     VX := VX shr 1, VF := carry 
					s=string.Format("{0:S}SHR  V{1:X} ",s,b1&0xf);
					break;
				case 7: // 8XY7     VX := VY - VX, VF := not borrow
					s=string.Format("{0:S}SUBN V{1:X}, V{2:D} ",s,(b1&0xf),(b2&0xf0)>>4);
					break;
			...
			case 9:
				...
				break;
			case 0xA: // ANNN     I := NNN 
				s=string.Format("{0:3,X}LD I, {1:3,X} ",s,code&0xfff);
				break;
			case 0xB: // BNNN     saut à NNN+V0
				s=string.Format("{0:3,X}JP V0, {1:3,X} ",s,code&0xfff);
				break;
			case 0xC: //CXKK VX=random_number&KK
				s=string.Format("{0:3,X}RND V[{1:X}], {2:2,X} ",s,b1&0xf,b2);
				break;
			case 0xD: //DXYN dessine le sprite pointé par I
				s=string.Format("{0:3,X}DRW V[{1:X}], V[{2:X}], {3:2,X} ",s,b1&0xf,(b2&0xf0)>>4,b2&0xf);
				break;
			case 0xE:
				switch(b2){
				case 0x9e: //EX9E, saute l'instruction suivante si la touche VX est enfoncée
					s=string.Format("{0:3,X}SKP V[{1:X}] ",s,b1&0xf);
					break;
				case 0xa1: //EXA1, saute l'instruction suivante si la touche VX n'est pas enfoncée
					s=string.Format("{0:3,X}SKNP V[{1:X}] ",s,b1&0xf);
					break;
				default:
					s=string.Format("{0:S}??? ",s);
					break;
